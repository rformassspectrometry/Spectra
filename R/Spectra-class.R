...
#'
#' @aliases Spectra-class coerce,Spectra,list-method
#'     coerce,Spectra,List-method
#'
#' @name Spectra
#'
#' @description
#'
#' The `Spectra` class encapsules data and meta-data for mass
...
#'
#' @param file for `readSpectra: `character` with the file names of the
...
#'
#' @param metadata for `Spectra` and `readSpectra`: `list` with
...
#'
#' @param object1 a `Spectra` object.
#'
...
#'
#' @param spectraData for `Spectra`: a [S4Vectors::DataFrame-class] object
...
#'
#' @param ... for `readSpectra`: additional parameters to be passed to
...
#'   a `DataFrame` or a `Spectra` with updated spectra metadata. Each
...
#'   containing information for one sample or file or a `Spectra` with
...
#'
#' - `[i]`: subset the object by spectra (`i`). Returns an `Spectra`,
...
#'
#' - `filterFile`: subset the object by file. Returns an `Spectra`.
#'
...
#'   Returns the filtered `Spectra` with all spectra with the specified
...
#'   the subsetted `Spectra`.
#'
#' - `splitByFile`: split an `Spectra` by file. The function returns a
#'   `list` of `Spectra` objects, each containing spectra from a single
...
#'   the `Spectra` with data manipulations applied and stored in the
#'   backend.
#'
...
#'   returns an `Spectra` with binned spectra.
#'
...
#'   [normalize()] for more details. The function returns the `Spectra`
...
#'   `Spectra` with centroided spectra.
#'
...
#'   `Spectra` with the reporter peaks removed.
#'
...
#'   `Spectra` object with smoothed spectra.
#'
...
#' intensity(mse_filt)
#'
#' ## Create an Spectra from two input files using the on-disk
#' ## BackendMzR backend
#' sf <- dir(system.file("sciex", package = "msdata"), full.names = TRUE)
#' mse <- readSpectra(sf, backend = BackendMzR())
#' mse
#'
...
#' ## `Spectra`. Below we change the backend from the default raw MS
...
#' validation function for Spectra
#'
#' @author Johannes Rainer
#'
#' @noRd
.validSpectra <- function(object) {
    msg <- NULL
    ...
    msg <- c(msg, .valid.processingQueue(object@processingQueue),
             .valid.Spectra.featureData(object@spectraData,
...
.valid.Spectra.featureData <- function(x, nsamples) {
    msg <- NULL
    if (nrow(x)) {
...
setClass(
    ...
    validity = .validSpectra
)

#' @rdname Spectra
Spectra <- function(x, spectraData, sampleData, metadata, ...) {
...
applyProcessingQueue <- function(x, BPPARAM = bpparam()) {
    if (!inherits(x, "Spectra"))
        stop("'x' is supposed to be an 'Spectra' object")
...
#' @rdname Spectra
setMethod("spectrapply", "Spectra", function(object,
                                                   FUN = NULL, ...,
...
#' @rdname Spectra
#' @name coerce,Spectra,list-method
...
#' @rdname Spectra
setMethod("bpi", "Spectra", function(object, initial = TRUE,
                                           BPPARAM = bpparam()) {
    if (initial)
...
#' @rdname Spectra
setMethod("chromatogram", "Spectra", function(object, rt, mz,
                                                    aggregationFun = "sum",
                                                    missing = NA_real_,
                                                    msLevel. = 1L,
                                                    BPPARAM = bpparam()){
...
#' @rdname Spectra
setMethod("centroided", "Spectra", function(object, na.fail = FALSE) {
...
#' @rdname Spectra
setMethod("featureData", "Spectra", function(object) {
    object@spectraData
})

#' @rdname Spectra
setMethod("fData", "Spectra", function(object) featureData(object))

#' @rdname Spectra
setReplaceMethod("featureData", "Spectra", function(object, value) {
...
#' @rdname Spectra
setMethod("featureNames", "Spectra", function(object) {
...
#' @rdname Spectra
setMethod("isCentroided", "Spectra",
...
#' @rdname Spectra
setMethod("length", "Spectra", function(x) {
    nrow(x@spectraData)
})

#' @rdname Spectra
setMethod("metadata", "Spectra",
          function(x, ...) {
...
#' @rdname Spectra
setMethod("peaksCount", "Spectra", function(object, BPPARAM = bpparam()) {
...
#' @rdname Spectra
setMethod("sampleData", "Spectra", function(object) {
    object@sampleData
})

#' @rdname Spectra
setReplaceMethod("sampleData", "Spectra", function(object, value) {
    if (!is(value, "DataFrame"))
        stop("'value' should be a 'DataFrame'")
...
##------------------------------------------------------------
#' @rdname Spectra
setMethod("[", "Spectra", function(x, i, j, ..., drop = TRUE) {
    if (!missing(j))
...
#' @rdname Spectra
setMethod("filterAcquisitionNum", "Spectra", function(object, n, file) {
...
#' @rdname Spectra
setMethod("filterFile", "Spectra", function(object, file) {
    if (missing(file))
        return(object)
...
#' @rdname Spectra
setMethod("filterPolarity", "Spectra", function(object, polarity.) {
    if (missing(polarity.))
        return(object)
...
#' @rdname Spectra
setMethod("filterRt", "Spectra", function(object, rt, msLevel.) {
...
#' @rdname Spectra
setMethod("splitByFile", c("Spectra", "factor"), function(object, f) {
...
#' @rdname Spectra
setMethod("bin", "Spectra", function(object, binSize = 1L, msLevel.) {
    if (missing(msLevel.))
...
#' @rdname Spectra
setMethod("compareSpectra", "Spectra",
...
#' @rdname Spectra
setMethod("estimateMzResolution", "Spectra", function(object, ...) {
    spectrapply(object, FUN = estimateMzResolution, ...)
})

#' @rdname Spectra
setMethod("estimateNoise", "Spectra",
...
##                    verbose = isMSnbaseVerbose(),
##                    ...) {
##               ## TODO: check if and how that works for Spectra!
##               method <- match.arg(method)
...
#' @rdname Spectra
setMethod("removePeaks", "Spectra", function(object, t = "min",
                                                   verbose = isMSnbaseVerbose(),
                                                   msLevel.) {
...
#' @rdname Spectra
setMethod("removeReporters", "Spectra",
