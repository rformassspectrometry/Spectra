...
#		spec_mat = spec_mat[no_dup,]
		...
		iso_grps = MetaboCoreUtils::isotopologues(
			...
			peaksData (spec)[[1]],
			ppm = ppm,
			isotopicSubstitutionMatrix(source = "HMDB_NEUTRAL")
		)
		not_iso = setdiff (1:nrow (spec_mat), unlist (iso_grps))
...
## Debug - this for some reason doesn;t work well:
		...
		spec_data$mz <- list(as.numeric(spec_mat[spec_mat[,"tag"] != 'p',"mz"]))
		spec_data$intensity <- list(as.numeric(spec_mat[spec_mat[,"tag"] != 'p',"intensity"]))
		spec_data$tag <- list(spec_mat[spec_mat[,"tag"] != 'p',"tag"])
		backendInitialize(MsBackendMemory(),spec_data)
	})
	## label list by original spectra peak IDs (even if precursorMz might have changed)
#browser()
	names(spec_list) <- spectraData(querySpectra)$peak_id
	## Remove empty spectra and spectra smaller than the minimum size
	spec_list <- spec_list[!sapply(spec_list,is.null) & sapply(spec_list,lengths) >= minSize]
	print ("Reduced spectra length:")
