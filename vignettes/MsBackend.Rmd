...
`DataFrame`. This would simplify the implementation of some replacement
methods. We thus re-implement the `backendInitialize` method supporting also to
initialize the backend with such a data frame and we also implement a helper
...
#' data type.
.sv_valid_data_type <- function(x) {
    sv <- coreSpectraVariables()[names(coreSpectraVariables()) %in% colnames(x)]
...
function that checks for the correct length of `value` since we need to perform
this test for every data replacement method.
...
backend. The implementation can be the same as for the `intensity<-` method.
