...
backends defined by the `r Biocpkg("MsBackendMgf")` or `r
Biocpkg("MsBackendMsp")` packages work in the same way and thus allow to import
MS data from these specific file formats. The `backendInitialize()` method of
the backend defined in the `r Biocpkg("MsBackendSql")` on the other hand takes
...
spectrum into a `matrix`. Also, for simplicity reasons, we accept only `c("mz",
"intensity")` for the `columns` parameter. This is the expected default behavior
...
tic values are usually provided by a spectra variable `"totIonCurrent"`. Thus,
for `initial = TRUE`, in our implementation below we return the value of such a
