...
`backendParallelFactor` function the backend can also *suggest* a `factor` that
...
splitting. `backendParallelFactor` for `MsBackendMzR` on the other hand returns
a `factor` based on the data files the data is stored in (i.e. based on the
`dataStorage` of the MS data).
...
The `backendParallelFactor` allows a backend to suggest a preferred way how the
backend could be split for parallel processing. See also the notes on parallel
processing above for more information. The default implementation returns
`factor()` (i.e. a `factor` of length 0) hence not suggesting any splitting:
