...
Certain backends (such as the `MsBackendMemory`) support also additional peaks
variables. Below we define *peak annotations* for each individual peak in the
peak matrices and assign that to a variable called `peak_anno` to the `Spectra`
object. Importantly, for peak variables, a value needs to be assigned to each
peak, even if it is `NA` or `empty`. In the example below we define a `list` of
`character` vectors with arbitrary IDs/names for each peak.

```{r}
sps2 <- sps
sps2$peak_anno <- list(c("a", NA_character_, "b", "c", "d"),
                       c("a", "b", "c", "d", "e", "f", "g"),
                       c("a", "b", "c", "d", "e", "f", "g", "h", "i"))
peaksVariables(sps2)
```

We can use the `peaksData` to extract the values for these additional peak
variable.
...
This works however **only** for `Spectra` that use a `MsBackendMemory` backend.
...
- `filterPrecursorMzRange`: retain (MSn) spectra with a precursor m/z within the
  provided m/z range.
- `filterPrecursorMzValues`: retain (MSn) spectra with precursor m/z value
...
normalized dot-product. For more details, see also [@rainer_modular_2022] or the
[SpectraTutorials](https://jorainer.github.io/SpectraTutorials) tutorial. Note
...
because the database connection can not be shared across processes. In such
cases parallel processing should be disabled *globally* for the R session using
`register(SerialParam())` or specifically in certain calls to functions of the
`Spectra` package by passing `BPPARAM = SerialParam()` to the function
(e.g. `peaksData(sps, BPPARAM = SerialParam())`).
...
- `r BiocStyle::Biocpkg("MsBackendMassbank")`: support for import/export and
  access of MassBank files/databases while still providing acceptable
  performance.
- [MsBackendSql](https://rformassspectrometry.github.io/MsBackendSql): stores
  all MS data in a SQL database and has thus a minimal memory footprint.
...
- [MsBackendMsp](https://rformassspectrometry.github.io/MsBackendMsp): import
