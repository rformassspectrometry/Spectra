...
Peak data can also be extracted with the `as.list` function that returns a list
of matrices, each with columns `"mz"` and `"intensity"` containing the m/z and
...
sps <- replaceIntensitiesBelow(sps, threshold = 10, value = 0)
```
...
```{r clean}
sps <- filterIntensity(sps, intensity = c(0.1, Inf))
```
...
is applied each time peak data is accessed (with the `as.list`, `mz` or
...
on peak data of the `sps` object persistent.

```{r applyProcessing}
length(sps@processingQueue)
...
which were then applied to the peak data and *written* to the data storage.


## Plotting `Spectra`

The `Spectra` package provides 3 functions to visualize spectra data:
`plotSpectra`, `plotSpectraOverlay` and `plotSpectraMirror`. The first function
plots the spectrum's m/z values against the intensity values. Below we plot the
4 spectra from the `sps` object using their names (as provided in spectra
...
value of each peak as label. In the example we define a function that accesses
information from each spectrum (`z`) and returns a `character` for each peak
with the label that should be used. Parameters `labelSrt`, `labelPos` and
`labelOffset` define the rotation of the label text and its position relative to
...
```{r plotspectraoverlay, fig.width = 6, fig.heigh = 6}
```

Note that also this function supports labeling of individual peaks.
