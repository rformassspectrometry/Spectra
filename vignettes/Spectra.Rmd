...
affecting the total number of spectra are (in alphabetic order):
...
- `filterPrecursorPeaks()`: removes peaks with either an m/z value matching the
  ...
  `mz = ">="`).
- `filterMzValues()`: subsets peaks within each spectrum keeping or removing
  (all) peaks matching provided m/z value(s) (given parameters `ppm` and
  `tolerance`).
