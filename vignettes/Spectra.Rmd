...
- `deisotopeSpectra`: deisotope each individual spectrum keeping only the
...
- `filterMzRange`: filter individual spectra keeping only peaks with an m/z
  within the provided m/z range.
- `filterMzValues`: filter individual spectra keeping or removing (all) peaks
  matching provided m/z value(s) (given parameters `ppm` and `tolerance`).
...
- `filterPrecursorIsotopes`: identifies precursor ions (from fragment spectra)
  ...
  spectra are returned as is.
- `filterPrecursorMaxIntensity`: filter spectra keeping, for groups of spectra
...
- `reduceSpectra`: filter individual spectra keeping only the largest peak for
