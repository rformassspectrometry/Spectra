...
retrieved with `dataOriginNames` and `dataStorageNames`, respectively.

```{r dataOriginNames}
dataOriginNames(sps)
dataStorageNames(sps)
basename(dataOriginNames(sps_sciex))
basename(dataStorageNames(sps_sciex))
```
...
origin in `dataOriginNames` or the actual name of the data origin.
...
sps_sciex[sps_sciex$dataOrigin == dataOriginNames(sps_sciex)[2] &
          sps_sciex$rtime >= 175 & sps_sciex$rtime <= 189]
```
...
function into a SQL condition to perform the subsetting in the database).
...
head(basename(dataOriginNames(sps_sciex)))
```
