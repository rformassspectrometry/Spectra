...
The parameter `peaksVariables` (currently only available for `MsBackendMemory`)
...
- `r Biocpkg("MsBackendRawFileReader")`: implements a backend for reading
  Orbitrap data using Thermo Fisher Scientific's NewRawFileReader .Net
  libraries. The package is generalizing the functionality introduced by the
  `r Biocpkg("rawrr")` package, see also [@kockmann_rawrr_2021].
