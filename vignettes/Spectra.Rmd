...
The `spectraData` function returns a `DataFrame` with the full data
for each spectrum, or with selected spectra variables (which can be specified
with the `columns` parameter). Below we extract the spectra data for variables
`"msLevel"`, `"id"` and `"name"`.
...
The unique elements (or levels) for `dataOrigin` and `dataStorage` can be
retrieved with `dataOriginLevels` and `dataStorageLevels`, respectively.

```{r dataOriginLevels}
dataOriginLevels(sps)
dataStorageLevels(sps)
basename(dataOriginLevels(sps_sciex))
basename(dataStorageLevels(sps_sciex))
```
...
in the measurement run. Note that we can either specify the index of the
origin in `dataOriginLevels` or the actual name of the data origin.

```{r filterfile-filterrt}
file_2 <- filterDataOrigin(sps_sciex, dataOrigin = 2)
length(file_2)
...
sps_sciex[sps_sciex$dataOrigin == dataOriginLevels(sps_sciex)[2] &
          sps_sciex$rtime >= 175 &
          sps_sciex$rtime <= 189]
```
...
head(basename(dataOriginLevels(sps_sciex)))
```
