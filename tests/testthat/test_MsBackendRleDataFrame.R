test_that("backendInitialize,MsBackendRleDataFrame works", {
    be <- MsBackendRleDataFrame()
    be <- backendInitialize(be)
    expect_true(validObject(be))
    expect_true(is(be, "MsBackendRleDataFrame"))
    expect_true(length(be@files) == 0)
    expect_true(all(names(.SPECTRA_DATA_COLUMNS) %in% colnames(be@spectraData)))

    be <- backendInitialize(be, files = NA_character_,
                            spectraData = DataFrame(fromFile = 1L))
    expect_true(validObject(be))
    expect_true(all(names(.SPECTRA_DATA_COLUMNS) %in% colnames(be@spectraData)))
    expect_equal(be@spectraData$msLevel, Rle(NA_integer_, 1))

    expect_error(backendInitialize(MsBackendRleDataFrame(),
...
test_that("acquisitionNum, MsBackendRleDataFrame works", {
    ...
    be <- backendInitialize(be, files = NA_character_,
                            spectraData = DataFrame(fromFile = 1L,
                                                    msLevel = c(1L, 2L)))
    expect_equal(be@spectraData$acquisitionNum, Rle(NA_integer_, 2))
    ...
    expect_equal(be@spectraData$acquisitionNum, 1:10)
})
