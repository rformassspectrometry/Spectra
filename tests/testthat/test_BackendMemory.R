...
test_that(".valid_spectrum_data_frame works", {
    x <- data.frame(mz = c(1.3, 1.5, 1.6), intensity = c(4, 5.3, 5))
    expect_null(.valid_spectrum_data_frame(x))
    expect_match(.valid_spectrum_data_frame("a"), "Spectra should be")
    expect_match(.valid_spectrum_data_frame(data.frame(a = 1:5, b = "a")),
                 "have columns")
    expect_match(.valid_spectrum_data_frame(data.frame(mz = 1, intensity = "b")),
                 "need to be of type numeric")
    expect_match(.valid_spectrum_data_frame(
        data.frame(mz = 2:1, intensity = c(1, 2))), "m/z values have to")
})
