...
- Fix in
  ...
  method for `MsBackendMemory` if only peaks variables are extracted.

### Change 1.19.10

- Add default implementation of methods for the `MsBackend` class (issue
  [\#338](https://github.com/rformassspectrometry/Spectra/issues/338)).
  This reduced code redundancy in *Spectra* and simplifies
...
- Add implementations of
  ...
  and `spectraVariableMapping<-` for `MsBackend` and `Spectra` objects.
...
- Add default implementations for `MsBackend` for replacement methods
  ...
  `isolationWindowUpperMz<-`, `msLevel<-`, `polarity<-`,
  `precursorMz<-`, `rtime<-` and `smoothed<-`. This reduces the amount
  of methods required to be implemented for new `MsBackend` classes.
...
- Addition of
  ...
  function. This function generates a `integer` index grouping MSn (MS
  level \> 1) spectra with their corresponding (unique) MS1 spectra
  based on acquisition order.
- Fix on
  [`cbind2()`](https://rformassspectrometry.github.io/Spectra/reference/combineSpectra.md)
  so that the output is a `Spectra` object and not the backend.

### Change 1.19.4

- Fix passing of `BPPARAM` parameter in
  [`peaksData()`](https://rformassspectrometry.github.io/Spectra/reference/spectraData.md)
  method for `Spectra`. Now is being passed down to `.peaksapply()`.

### Change in 1.19.3

- Fix in
  [`estimatePrecursorMz()`](https://rformassspectrometry.github.io/Spectra/reference/estimatePrecursorMz.md)
  that would ignore parameter `BPPARAM`.
...
- Allow parameter `msLevel. = integer()` for
  [`filterRt()`](https://rformassspectrometry.github.io/Spectra/reference/filterMsLevel.md)
  to filter spectra of **all** MS levels. This was
  `msLevel. = uniqueMsLevels()`, which, depending on the backend, can be
  computationally intense. Add related unit tests to the unit test
  suite.

### Change in 1.17.8

- Add parameter `return.type` to
  [`peaksData()`](https://rformassspectrometry.github.io/Spectra/reference/spectraData.md).
...
- Add new
  ...
  function that allows to add eventually missing *core* spectra
  variables (with the correct data type) to a data frame.
...
- Move generics
  ...
  and `applyproceesing()` to *ProtGenerics*. Required *ProtGenerics*
  version 1.39.2 or higher. These were moved to be able to implement
  them in the *Chromatograms* package.
...
- Fix
  [`cbind2()`](https://rformassspectrometry.github.io/Spectra/reference/combineSpectra.md)
  unit test for backends that fails if the number of spectra in the
  tested backend is (by chance) equal to 4.

### Change in 1.17.2

- Add
  [`cbind2()`](https://rformassspectrometry.github.io/Spectra/reference/combineSpectra.md)
  method to easily add multiple `spectraVariables` and their content to
  the `spectraData` of a `Spectra` object. See also
  [issue](https://github.com/rformassspectrometry/Spectra/issues/342)
  [\#342](https://github.com/RforMassSpectrometry/Spectra/issues/342)
...
- Add `precursorMz<-` method
  [issue](https://github.com/rformassspectrometry/Spectra/issues/336)
  [\#336](https://github.com/RforMassSpectrometry/Spectra/issues/336).
...
- Add new `extractSpectra()` generic and implementation for `MsBackend`.
  Fixes
  [issue](https://github.com/rformassspectrometry/MsBackendMetaboLights/issues/5)
  [\#5](https://github.com/RforMassSpectrometry/Spectra/issues/5).
...
- Refactor the
  ...
  constructor method: better support for initialization of backends that
  define their own specific parameters.

### Changes in 1.15.7

- Change
  [`estimatePrecursorIntensity()`](https://rformassspectrometry.github.io/Spectra/reference/estimatePrecursorIntensity.md)
  to a method to avoid overrides/clashes with the same-named
...
- Fix in
  ...
  for `MsBackendMzR`: ensure peaks variables `"mz"` and `"intensity"`
...
- Add new
  ...
  function to filter mass peaks by ranges on numeric spectra or peak
  variables.
...
- New
  ...
  function to convert a (small) `Spectra` object into a long
  `DataFrame`.

### Changes in 1.15.2

- Add `dataStorageDataPath()` and `dataStorageDataPath<-` methods to
  allow updating/adapting the path of the data storage files of backends
  supporting that
  [issue](https://github.com/rformassspectrometry/Spectra/issues/321)
  [\#321](https://github.com/RforMassSpectrometry/Spectra/issues/321).
...
- Improve documentation for
  ...
  [issue](https://github.com/rformassspectrometry/Spectra/issues/320)
...
- Add
  [`estimatePrecursorMz()`](https://rformassspectrometry.github.io/Spectra/reference/estimatePrecursorMz.md)
  function to *estimate* the precursor m/z for DDA fragment spectra from
  previous MS1 spectra
  [issue](https://github.com/rformassspectrometry/Spectra/issues/315)
...
- Add
  ...
  functions to allow filtering of a Spectra object based on ranges or
  similarities of any existing `spectraData` variables.
...
- Fix issue in `setBackend` that might cause chunk-wise processing to be
  not run.

### Changes in 1.13.2

- Add possibility to enable and perform chunk-wise (parallel) processing
  to `Spectra`: add functions `processingChunkSize`,
  `backendParallelFactor` and `processingChunkFactor` to set or get
  definition of chunks for parallel processing. All functions working on
  peaks data use this mechanism which is implemented in the internal
  `.peaksapply` function. The `Spectra` object gains a new slot
  `"processingChunkSize"` that is used to define the size of the
  processing chunks for the `Spectra`. See also
  [issue](https://github.com/rformassspectrometry/Spectra/issues/304)
...
- Fix issue with `bin` function (see
  [issue](https://github.com/rformassspectrometry/Spectra/issues/303)
  [\#302](https://github.com/RforMassSpectrometry/Spectra/issues/302)).
...
- Fix issue with `filterFourierTransformArtefacts` function (see
  [issue](https://github.com/rformassspectrometry/Spectra/issues/302)
  [\#302](https://github.com/RforMassSpectrometry/Spectra/issues/302)).
  Thanks Adriano Rutz for reporting.
...
- `peaksData,MsBackendMemory` returns a `data.frame` if additional peak
  ...
  returned.
- `peaksData,Spectra` returns either a `matrix` or `data.frame` and
  ensures the peak data is correctly subset based on the lazy evaluation
  processing queue.
- `$,Spectra` to access peak variables ensures the lazy evaluation queue
...
- Fix in `compareSpectra` to also pass parameters `ppm` and `tolerance`
  to the peak similarity calculation functions `FUN`: this allows to use
...
- Add `joinPeaksNone` to skip the peak matching in `compareSpectra` if
  the similarity scoring function performs its own peak matching.
...
- Add `scalePeaks` function (see
  [issue](https://github.com/rformassspectrometry/Spectra/issues/291)
...
- Add example for filtering precursor m/z peaks from fragment spectra to
  the vignette.
...
- Fix bug in
  ...
  and PR
  [\#277](https://github.com/RforMassSpectrometry/Spectra/issues/277)).
...
- Fix issue with `filterMzValues` that would only keep (or remove) the
  ...
  `tolerance`). Issue
  [\#274](https://github.com/rformassspectrometry/Spectra/issues/274).
...
- Add the `backendBpparam` method that allows to evaluate whether a
  `MsBackend` supports the provided (or the default)
  `BiocParallel`-based parallel processing setup.
- Minor tweaks in the internal `.peaksapply` function to avoid
  splitting/merging of data if not needed (e.g. if no parallel
  processing is performed).
- Minor tweaks in spectra comparison functions to avoid repeated calling
  of functions in loops.
...
- Support `setBackend` with `MsBackendMemory` for an empty `Spectra`
  object (issue
  [\#268](https://github.com/rformassspectrometry/Spectra/issues/268)).
...
- Add a vignette describing how to build a `MsBackend` from scratch
  (issue
  [\#262](https://github.com/RforMassSpectrometry/Spectra/issues/262)).
...
- Fix bug in
  [`containsMz()`](https://rformassspectrometry.github.io/Spectra/reference/addProcessing.md)
  when `mz` isn’t ordered (see
  [\#258](https://github.com/RforMassSpectrometry/Spectra/issues/258)).
...
- Force serial processing in some unit tests to avoid potential failures
  on some Bioconductor build and check servers (under some
  circumstances).
...
- Add `uniqueMsLevels` function to allow more efficient,
  backend-specific, implementations for retrieving unique MS levels from
  a data set.
...
- Add parameters `ppm` and `tolerance` to `PrecursorMzParam` (for
  neutral loss calculation) and add option
  `filterPeaks = "removePrecursor"`.
...
- Only calculate number of peaks per spectra if the processing queue of
  the `Spectra` is not empty. Otherwise call the backend’s
  implementation (issue
  [MsBackendSql](https://github.com/rformassspectrometry/MsBackendSql/issues/31)
  [\#31](https://github.com/RforMassSpectrometry/Spectra/issues/31)).
...
- Add `filterFourierTransformArtefacts` function to remove fast fourier
  artefact peaks seen on e.g. Orbitrap instruments (issue
  [\#223](https://github.com/RforMassSpectrometry/Spectra/issues/223)).
...
- New features:
  ...
  now check for duplicated keys in `x` (throws an error) and `y` (thows
  a warning).
...
- Add fix from `MSnbase` (issue
  [\#170](https://github.com/lgatto/MSnbase/issues/170)) to `Spectra`:
  on macOS require reading also the spectrum header before reading the
  peaks data.
...
- `filterMzValues` supports also removing peaks matching specified m/z
  values (issue
  [\#209](https://github.com/rformassspectrometry/Spectra/issues/209)).
...
- Fix issue
  [\#187](https://github.com/rformassspectrometry/Spectra/issues/187).
...
- Pass spectras’ precursor m/z to the `MAPFUN` in `compareSpectra`;
  issue
  [\#171](https://github.com/rformassspectrometry/Spectra/issues/171).
- Add `joinPeaksGnps` to perform a peak matching between spectra similar
  to the one performed in GNPS (issue
  [\#171](https://github.com/rformassspectrometry/Spectra/issues/171)).
...
- Fix in `spectraData<-,Spectra` to avoid removing m/z and intensity
  values (issue
  [\#146](https://github.com/rformassspectrometry/Spectra/issues/146)).
...
- Fix in `Spectra,character` constructor to ensure the backend is
  changed even if `source` inherits from `backend` (issue
...
- Support `intensity` in `filterIntensity` method to be a function to
  enable peak intensity-based filtering of spectra (issue
...
- Change parameter `source` in `Spectra,character` to `MsBackendMzR` and
  set parameter `backend = source`. Thus by default, the import backend
